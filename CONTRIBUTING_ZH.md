# 貢獻指南

感謝您對 `line-pay-offline-v4` 的興趣！我們歡迎社群的貢獻。

[English](./CONTRIBUTING.md) | 繁體中文

## 目錄

- [行為準則](#行為準則)
- [如何貢獻？](#如何貢獻)
- [開發環境設定](#開發環境設定)
- [開發流程](#開發流程)
- [程式碼規範](#程式碼規範)
- [提交規範](#提交規範)
- [Pull Request 流程](#pull-request-流程)
- [測試](#測試)
- [文件](#文件)

## 行為準則

本專案遵循行為準則，所有貢獻者都應遵守。請在所有互動中保持尊重和建設性。

## 如何貢獻？

### 回報 Bug

在建立 Bug 報告之前，請先檢查現有的 issues 以避免重複。建立 Bug 報告時，請包含：

- **清晰的標題**：簡要描述問題
- **描述**：問題的詳細描述
- **重現步驟**：逐步說明如何重現
- **預期行為**：您期望發生的事情
- **實際行為**：實際發生的事情
- **環境**：作業系統、Node.js/Bun 版本、套件版本
- **程式碼範例**：能重現問題的最小程式碼（如適用）

### 建議功能

功能建議透過 GitHub issues 追蹤。建立功能建議時：

- **使用清晰且描述性的標題**
- **提供詳細描述**建議的功能增強
- **解釋為什麼這個增強功能會有用**
- **列出您考慮過的替代方案**

### Pull Requests

我們積極歡迎您的 pull requests：

1. Fork 這個 repo 並從 `main` 建立您的分支
2. 如果您新增了需要測試的程式碼，請新增測試
3. 如果您更改了 API，請更新文件
4. 確保測試套件通過
5. 確保您的程式碼遵循程式碼規範
6. 提交 pull request

## 開發環境設定

### 先決條件

- [Bun](https://bun.sh/) v1.3.4 或更新版本
- [Git](https://git-scm.com/)
- 程式碼編輯器（推薦 VS Code）

### 設定步驟

1. **Fork 並複製儲存庫**

```bash
git clone https://github.com/YOUR_USERNAME/line-pay-offline-v4.git
cd line-pay-offline-v4
```

2. **安裝依賴**

```bash
bun install
```

3. **執行測試確保一切正常**

```bash
bun test
```

## 開發流程

### 1. 建立功能分支

```bash
git checkout -b feature/your-feature-name
# 或
git checkout -b fix/your-bug-fix
```

### 2. 進行修改

- 撰寫乾淨、可讀的程式碼
- 遵循程式碼規範
- 為新功能新增測試
- 視需要更新文件

### 3. 測試您的修改

```bash
# 執行型別檢查
bun run typecheck

# 執行 linting
bun run check

# 執行測試
bun test

# 執行測試並產生覆蓋率報告
bun run test:coverage

# 建置專案
bun run build
```

### 4. 提交您的修改

```bash
git add .
git commit -m "type: description"
```

請參閱[提交規範](#提交規範)了解提交訊息格式。

### 5. 推送並建立 Pull Request

```bash
git push origin feature/your-feature-name
```

然後在 GitHub 上建立 Pull Request。

## 程式碼規範

### TypeScript

- 所有原始碼使用 TypeScript
- 啟用嚴格模式
- 避免使用 `any` 型別
- 使用 JSDoc 註解記錄公開 API

### 程式碼風格

我們使用 Biome 進行格式化和 linting。遵循以下慣例：

- **縮排**：2 個空格（由 Biome 處理）
- **分號**：不需要（已在專案中設定）
- **引號**：字串使用單引號
- **命名慣例**：
  - `PascalCase` 用於類別和型別
  - `camelCase` 用於函式和變數
  - `UPPER_CASE` 用於常數

### 範例

```typescript
/**
 * 驗證 LINE Pay 交易 ID
 * @param transactionId - 要驗證的交易 ID
 * @throws {LinePayValidationError} 當格式無效時
 */
export function validateTransactionId(transactionId: string): void {
  if (!TRANSACTION_ID_REGEX.test(transactionId)) {
    throw new LinePayValidationError(
      `Invalid transactionId format: expected 19-digit number, got "${transactionId}"`
    )
  }
}
```

## 提交規範

我們遵循 [Conventional Commits](https://www.conventionalcommits.org/) 規範。

### 提交訊息格式

```
<type>(<scope>): <subject>

<body>

<footer>
```

### 型別

- **feat**：新功能
- **fix**：Bug 修復
- **docs**：僅文件變更
- **style**：程式碼風格變更（格式化、分號等）
- **refactor**：程式碼重構（既不修復 bug 也不新增功能）
- **perf**：效能改進
- **test**：新增或更新測試
- **chore**：建置流程或輔助工具的變更

### 範例

```bash
# 功能
git commit -m "feat: 新增付款狀態輪詢工具"

# Bug 修復
git commit -m "fix: 修正退款請求中的 orderId URL 編碼"

# 文件
git commit -m "docs: 更新 checkPaymentStatus 的 API 參考"

# 重構
git commit -m "refactor: 將裝置標頭提取為獨立方法"
```

## Pull Request 流程

1. **更新文件**：確保 README 已更新
2. **新增測試**：所有新功能必須包含測試
3. **通過所有檢查**：確保型別檢查、linting、測試和建置通過
4. **清理提交歷史**：視需要 squash 或 rebase
5. **參考 Issues**：在 PR 描述中連結相關 issues
6. **請求審查**：標記維護者進行審查

### PR 標題格式

遵循與提交訊息相同的格式：

```
feat: 新增用於解析的工具函式
fix: 解決逾時錯誤處理
docs: 改進 README 範例
```

## 測試

### 撰寫測試

- 將測試放在 `tests/*.test.ts` 檔案中
- 使用 Bun 的測試執行器
- 目標達到高程式碼覆蓋率（100% 目標）
- 測試邊界情況和錯誤條件

### 測試範例

```typescript
import { describe, expect, test } from 'bun:test'
import { LinePayOfflineClient } from '../src'

describe('LinePayOfflineClient', () => {
  describe('checkPaymentStatus', () => {
    test('should call API with correct URL', async () => {
      // 測試實作
    })
    
    test('should URL encode special characters', async () => {
      // 測試實作
    })
  })
})
```

### 執行測試

```bash
# 執行所有測試
bun test

# 以監看模式執行測試
bun test --watch

# 執行測試並產生覆蓋率
bun run test:coverage

# 執行特定測試檔案
bun test tests/LinePayOfflineClient.test.ts
```

## 文件

### README 更新

- 保持 README.md 與所有功能同步
- 為新功能包含程式碼範例
- 更新 API 參考區段

### 程式碼文件

- 使用 JSDoc 記錄公開 API
- 記錄參數、回傳型別和例外
- 在註解中包含使用範例

### 範例

```typescript
/**
 * 使用客戶的一次性條碼請求付款
 * 
 * @param request - 付款請求資料
 * @returns 包含交易 ID 的付款回應
 * 
 * @example
 * ```typescript
 * const payment = await client.requestPayment({
 *   amount: 100,
 *   currency: 'TWD',
 *   oneTimeKey: '12345678901245678',
 *   orderId: 'ORDER-001'
 * })
 * ```
 */
async requestPayment(request: PaymentRequest): Promise<PaymentResponse> {
  // 實作
}
```

## 問題？

如果您對貢獻有任何問題，歡迎：

- 開啟 [GitHub Issue](https://github.com/CarlLee1983/line-pay-offline-v4/issues/new)
- 建立帶有「question」標籤的 issue

## 授權

提交貢獻即表示您同意您的貢獻將以 MIT 授權條款授權。

---

感謝您對 `line-pay-offline-v4` 的貢獻！🎉
